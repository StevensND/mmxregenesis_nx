...
int mmx_engine_threads(Thread **out_thr, void **out_entry, int max) {
  int n = 0;
...
// ---------------------------------------------------------------------------
// MMX Regenesis (Godot 4.7) extra imports not present in the 4.6.dev4 base.
...
void mmx_resolve_imports(so_module *mod) {
  so_relocate(mod);
...
uintptr_t mmx_find_import(const char *name) {
